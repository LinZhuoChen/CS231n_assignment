...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss = loss + 0.5 * self.reg * np.sum(np.square(self.params['W2'])) + \
                0.5 * np.sum(self.reg * np.square(self.params['W1'])) 
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if i < self.num_layers - 1:
                hidden_features[i + 1], caches[i] = affine_relu_forward(hidden_features[i],
                                                                        self.params['W%d' % (i + 1)],
                                                                        self.params['b%d' % (i + 1)])
            else:
                hidden_features[i + 1], caches[i] = affine_forward(hidden_features[i],
                                                                            self.params['W%d' % (i + 1)],
                                                                            self.params['b%d' % (i + 1)])
        scores = hidden_features[self.num_layers]
